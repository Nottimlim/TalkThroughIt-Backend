...
export const updateAppointment = async (req, res) => {
  try {
    ...
    if (["cancelled", "rejected"].includes(req.body.status)) {
      const canCancel = appointment.canCancel();
      if (!canCancel) {
        return res.status(400).json({
          error:
            "Cannot cancel/reject appointment with less than 24 hours notice",
        });
      }
    }
...
export const cancelAppointment = async (req, res) => {
  try {
    ...
    appointment.cancellationReason = `Cancelled by ${req.user.type}: ${
...
export const getProviderAppointments = async (req, res) => {
  try {
    ...
    // Build query
    // In getProviderAppointments controller
    const query = {
      provider: req.user._id,
      datetime: { $gte: new Date() },
    };
    ...
    const appointments = await Appointment.find(query)
      .populate({
        path: "client",
        select:
          "firstName lastName email location insuranceProvider therapyGoals",
      })
